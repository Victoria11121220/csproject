...
impl Debug for Node {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Node {{ id: {}, concrete_node: {:?} }}",
            self.id, self.concrete_node
        )
    }
}
...
impl Node {
    ...
    pub async fn update(&mut self, inputs: GraphPayload) {
        self.internal_state = Some(match inputs {
            ...
            GraphPayload::Mixed(mixed) => match self.compute_mixed(&mixed).await {
                Ok(result) => Ok(result),
...
mod tests {
    ...
    fn deref() {
        ...
        internal_mock
            .expect_set_actual_handles()
            .return_const(Ok(()));

        let mut node = mock_node(internal_mock);
        assert!(node
            .set_actual_handles(HashSet::new(), HashSet::new())
            .is_ok());
    }

    #[tokio::test]
    async fn update_bypass() {
        let canary_objects: GraphPayloadObjects =
            vec![("canary".to_string(), serde_json::json!(1.0))]
                .into_iter()
                .collect();
        let canary_collections: GraphPayloadCollections =
            vec![("canary".to_string(), vec![serde_json::json!(1.0)])]
                .into_iter()
                .collect();
        ...
        internal_mock
            ...
            .return_const(Ok(GraphPayload::Objects(
    ...
    async fn clear_get_data() {
        ...
        assert!(matches!(
            node.get_data(),
            Some(Ok(GraphPayload::Objects(_)))
        ));
        node.clear();
        assert!(node.get_data().is_none());
    }
        ...
        internal_mock
            ...
            .returning(|collections| {
                ...
                match collection_lengths.len() {
                    ...
                    _ => Err(NodeError::InvalidInputError(
    ...
    async fn mixed_fallback_to_objects() {
        let canary: GraphPayloadMixed = vec![
            ...
            (
                "test".to_string(),
                NodeData::Collection(vec![serde_json::json!(2.0)]),
            ),
        ]
        .into_iter()
        .collect();
        ...
        let mut internal_mock = MockInternalNode::new();
        internal_mock.expect_compute_mixed().return_const(Err(
            NodeError::ComputationUnavailableError("Mixed input not supported".to_string()),
        ));
        internal_mock
            .expect_compute_objects()
    ...
    async fn mixed_fallback_norm_error() {
        ...
        assert!(matches!(
            node.get_data(),
            Some(Err(NodeError::InputNormalizationError(_)))
        ));
    }
}